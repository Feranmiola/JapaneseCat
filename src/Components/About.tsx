
const About = () => {
  return (
    <div className="w-full absolute mt-[55rem] z-40">

        <div className="pt-[35rem] pb-[10rem]">
            <div className="flex flex-col space-y-[15rem] items-center justify-center">
                <div className="flex flex-col items-center justify-center space-y-2">
                    <p className="font-electroharmonix text-[48px] text-goldText">About Japanese Cat</p>
                    <p className="text-[16px] text-white w-[753px] text-center font-thin tracking-[10%]">MEET MANEKI NEKO, THE ICONIC JAPANESE LUCKY CAT! KNOWN FOR ITS BECKONING PAW, MANEKI NEKO IS BELIEVED TO BRING GOOD FORTUNE AND PROSPERITY. THIS CHARMING FELINE IS SET TO BECOME THE FACE OF A REVOLUTIONARY NEW TOKEN, CAPTURING THE ESSENCE OF LUCK AND SUCCESS.</p>
                </div>

                <img
                src="./assets/catAbout.svg"
                />
            </div>

        </div>
        
    </div>
  )
}

export default About